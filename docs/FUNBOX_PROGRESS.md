...
## Stage 18 — localized product name cleanup
...
## Stage 21 — runtime Clipbox account flow (in progress)

- Continued from the existing branch without replacing the prior APK or resetting the PR. The authorized APK confirms `POST /api/auth/login` with `username`, `password`, and a persistent UUID `deviceId`; the response has `token` and `userId`. Clipbox stores the session in `EncryptedSharedPreferences`, removes it on logout, and has no observed refresh endpoint.
- Added a signed login request to `ClipboxApi`, a Hilt `ClipboxAuthRepository`, encrypted runtime token storage, session validation, logout, and a native account screen reachable from Clipbox Home and the Movies/Series settings category. Passwords and tokens are not logged, committed, placed in a URL, or added as build properties. Guest catalog browsing remains independent of account state.
- Added focused login protocol tests with fake credentials and an intercepted HTTP client. `:data:testDebugUnitTest --tests com.streamvault.data.remote.clipbox.ClipboxApiTest` passed. Full app compilation and an on-device account screen check remain pending. A real account login cannot be verified without a user's account credentials, which are not present in the APK.
- Rechecked the signed Clipbox configuration on its original host: HTTP 200; account check returns 401 for a missing user token, as expected. `/api/scrapers` and sample `/api/movie` and `/api/tv` paths returned 404. The current configuration has null `server_order`, `source_hosts`, and `server_descriptions` values. No source endpoint has yet returned a playable stream, so source retrieval and VOD playback remain open.
